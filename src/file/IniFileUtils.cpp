...
ErrorCode IniFileUtils::parseFile(std::string_view file, IniFileData &outData) {
...
ErrorCode IniFileUtils::serializeFile(
  const std::string& filePath, const IniFileData &data) {
  std::ofstream fileStream(filePath.c_str());
  if (!fileStream) {
    LOGERR("Error, file: [%s] could not be opened", filePath.c_str());
    return ErrorCode::FAILURE;
  }

  for (const auto& [sectionName, sectionData] : data) {
    fileStream << '[' << sectionName << "]\n";
    for (const auto& [entryName, entryData] : sectionData) {
      fileStream << entryName << " = " << entryData << '\n';
    }
    fileStream << '\n';
  }
...
bool IniFileUtils::getKeyValueInt(const IniFileSection &section,
...
bool IniFileUtils::getKeyValueString(const IniFileSection &section,
                                     const std::string &identifier,
                                     std::string &outValue) {
  auto it = section.find(identifier);
  if (it == section.end()) {
...
void IniFileUtils::print(const IniFileData &data) {
